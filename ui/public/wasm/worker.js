...
/**
 * **DO NOT EDIT**
 * This bind function is provided by golang which sets up the required binding for Golang WASM bindings to work
 */
const bind = () => {
...
const init = () => {
  ...
  WebAssembly.instantiateStreaming(
    ...
    .then((result) => {
      ...
      self.postMessage({ command: "init", successful: true, data: null });
    })
    .catch((err) => {
      console.error(err);
    });
...
const parse = (file) => {
  ...
  reader.addEventListener("load", (e) => {
    ...
    const goRes = self.readXLSXFile(u8array);

    /** @type {{ successful: boolean, size: number }} */
    const { successful, size } = JSON.parse(goRes);

    self.postMessage({ command: "parse", successful, data: { size } });
  });
